...
```{r}
K_hospital = Kest(hospitals_seoul, correction = "Ripley")
plot(K_ck, . -r ~ r, ylab= "K(d)-r", xlab = "d(m)")
```

```{r}
K_hostpital.csr <- envelope(hospitals_seoul, Kest, nsim = 39, rank = 1, glocal=TRUE)
```

```{r}
plot(K_hopital.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```

-   do k function for hospital

-   L function for clinic and hospital
